...
func initSyncTestDefault(t *testing.T) (*chain.DefaultSyncer, chain.Store, repo.Repo, *th.TestFetcher) {
	...
	return initSyncTest(t, con, initGenesis, cst, bs, r, false)
}
...
func initSyncTestCaughtUp(t *testing.T) (consensus.Protocol, *chain.DefaultSyncer, chain.Store, repo.Repo, *th.TestFetcher) {
	processor := th.NewTestProcessor()
	powerTable := &th.TestView{}
	r := repo.NewInMemoryRepo()
	bs := bstore.NewBlockstore(r.Datastore())
	...
	con := consensus.NewExpected(cst, bs, processor, powerTable, genCid, verifier)
	requireSetTestChain(t, con, false)
	sync, store, repo, tf := initSyncTest(t, con, initGenesis, cst, bs, r, false)
...
func initSyncTestWithPowerTable(t *testing.T, powerTable consensus.PowerTableView) (*chain.DefaultSyncer, chain.Store, consensus.Protocol, *th.TestFetcher) {
	...
	con := consensus.NewExpected(cst, bs, processor, powerTable, genCid, verifier)
	requireSetTestChain(t, con, false)
	sync, testchain, _, fetcher := initSyncTest(t, con, initGenesis, cst, bs, r, false)
	return sync, testchain, con, fetcher
}

func initSyncTest(t *testing.T, con consensus.Protocol, genFunc func(cst *hamt.CborIpldStore, bs bstore.Blockstore) (*types.Block, error), cst *hamt.CborIpldStore, bs bstore.Blockstore, r repo.Repo, caughtUp bool) (*chain.DefaultSyncer, chain.Store, repo.Repo, *th.TestFetcher) {
	ctx := context.Background()
