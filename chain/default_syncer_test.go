...
func initSyncTestDefault(t *testing.T) (*chain.DefaultSyncer, *chain.DefaultStore, repo.Repo, *th.TestFetcher) {
...
// needed to run tests.  It also sets the global test variables appropriately.
func initSyncTestWithPowerTable(t *testing.T, powerTable consensus.PowerTableView) (*chain.DefaultSyncer, *chain.DefaultStore, consensus.Protocol, *th.TestFetcher) {
	processor := th.NewTestProcessor()
	r := repo.NewInMemoryRepo()
	bs := bstore.NewBlockstore(r.Datastore())
	cst := hamt.NewCborStore()
...
func initSyncTest(t *testing.T, con consensus.Protocol, genFunc func(cst *hamt.CborIpldStore, bs bstore.Blockstore) (*types.Block, error), cst *hamt.CborIpldStore, bs bstore.Blockstore, r repo.Repo) (*chain.DefaultSyncer, *chain.DefaultStore, repo.Repo, *th.TestFetcher) {
	ctx := context.Background()
